...
function renderTwinRoom(r, canvas) {
  ...
  const plugsHTML = r.plugs.map(p => {
    ...
    const deviceLabel = p.device_name ? `<div class="pwatt device-name" style="color:var(--text3);font-size:7px;">${p.device_name}</div>` : '';
    return `
      <div class="plug-node ${cls}" data-plug-id="${p.id}" data-plug-code="${p.plug_code}" onclick="openPlugDetailDrawer('${p.id}')">
        <div class="pico">${emoji}<div class="pring"></div></div>
        <div class="pname">${p.name}</div>
        ${deviceLabel}
        <div class="pwatt power-watt">${isOn && p.current_power_w != null ? p.current_power_w.toFixed(0)+'W' : '—'}</div>
...
function connectPlugWebSocket(plugCode) {
  ...
  const ws = new WebSocket(`${protocol}//${window.location.host}/ws/plug/${plugCode}/`);
  ...
  ws.onclose = function() {
    ...
    setTimeout(() => {
      if (currentPlugs.find(p => p.plug_code === plugCode)) {
        connectPlugWebSocket(plugCode);
      }
    }, 3000);
  };
...
function connectAllPlugWebSockets() {
  currentPlugs.forEach(p => {
    if (p.plug_code && !plugWebSockets[p.plug_code]) {
      connectPlugWebSocket(p.plug_code);
    }
  });
}

function disconnectAllPlugWebSockets() {
  Object.values(plugWebSockets).forEach(ws => {
    if (ws.readyState === WebSocket.OPEN || ws.readyState === WebSocket.CONNECTING) {
      ws.close();
    }
  });
  Object.keys(plugWebSockets).forEach(key => delete plugWebSockets[key]);
}

function handlePlugUpdate(plugCode, data) {
  console.log('Handling plug update for', plugCode, data);
  
  const plugNode = document.querySelector(`[data-plug-code="${plugCode}"]`);
  
  if (data.event === 'status') {
    if (plugNode) {
      const isOn = data.is_on;
      const isOnline = data.online_status === 'online';
      plugNode.classList.remove('on', 'off');
      plugNode.classList.add(!isOnline ? 'off' : isOn ? 'on' : 'off');
    }
  ...
  if (data.event === 'energy') {
    if (plugNode) {
      const powerEl = plugNode.querySelector('.power-watt');
      if (powerEl && data.watts != null) {
        powerEl.textContent = data.watts.toFixed(0) + 'W';
      }
    }
    updateDashboardStats();
  }
  
  if (data.event === 'relay_on' || data.event === 'relay_off') {
    if (plugNode) {
      const isOn = data.event === 'relay_on';
      plugNode.classList.remove('on', 'off');
      plugNode.classList.add(isOn ? 'on' : 'off');
    }
    const plug = currentPlugs.find(p => p.plug_code === plugCode);
    if (plug) {
      plug.is_on = data.event === 'relay_on';
    }
    updateDashboardStats();
  }
  ...
  if (data.event === 'nfc_scan' && data.known) {
    if (plugNode) {
      const deviceNameEl = plugNode.querySelector('.device-name');
      if (deviceNameEl && data.device_name) {
...
function updateDashboardStats() {
  ...
  const onPlugs = currentPlugs.filter(p => p.is_on);
  const totalPower = onPlugs.reduce((sum, p) => sum + (p.current_power_w || 0), 0);
  
  const qsPower = document.getElementById('qs-power');
  const qsOnline = document.getElementById('qs-online');
  const sbW = document.getElementById('sb-w');
  
  if (qsPower) qsPower.textContent = totalPower.toFixed(0);
  if (qsOnline) qsOnline.textContent = onPlugs.length;
  if (sbW) sbW.textContent = `${totalPower.toFixed(0)} W`;
}

// Initialize WebSocket connections when house changes
document.addEventListener('houseChanged', async function() {
  disconnectAllPlugWebSockets();
  ...
  if (res.ok) {
    ...
    currentPlugs = plugs;
    connectAllPlugWebSockets();
  }
});

// Connect WebSockets after initial house load
const originalSetSetActiveHouse = setActiveHouse;
setActiveHouse = function(id, name, role) {
  originalSetSetActiveHouse(id, name, role);
  if (id) {
    setTimeout(() => connectAllPlugWebSockets(), 500);
  }
};

// Also connect on initial load after loadPlugs
const originalLoadPlugs = loadPlugs;
loadPlugs = async function() {
  await originalLoadPlugs();
  connectAllPlugWebSockets();
};
