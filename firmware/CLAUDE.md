...
| PZEM RX (ESP receives) | 16 | Serial2 RX2 |
| PZEM TX (ESP sends) | 17 | Serial2 TX2 |
...
| `PZEM_RX` | `16` | Serial2 RX |
| `PZEM_TX` | `17` | Serial2 TX |
| `STATUS_INTERVAL_MS` | `1000` | `/status` publish interval |
...
**`status`** — published every 1s and on connect:
```json
...
- **Library**: olehs/PZEM004T, Hardware Serial2
- **Read interval**: every 1s
- **Publish logic**: same as NFC — publish on 1% change in any field, or 1min heartbeat
...
- **Normal state**: toggles relay → publishes `relay_on` or `relay_off` event
- **Fault/block state**: triggers full software restart (`ESP.restart()`)
...
- WiFi connect → MQTT connect → publish `status` online → begin loop
...
**Change LED patterns**
Modify `ledPattern()` — each pattern is a self-contained non-blocking sequence using `millis()`.

**Swap NFC library**
Replace PN532 init and `readNfcUid()` function. Keep debounce logic untouched.
